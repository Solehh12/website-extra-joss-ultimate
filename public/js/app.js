(function () {
  ...
  function cacheData() { /* Supabase is the single source of truth; do not cache the full dataset in localStorage. */ }
  ...
  function adminSystemDashboard(){
    ...
    const storageLabel=health.storage==='supabase'?'Supabase':'Belum diperiksa';
  ...
  function advancedSystemStatus() {
    const health=state.systemHealth||{},counts=health.counts||{},connected=Boolean(window.ExtraJossBackend?.hasToken?.()),storageLabel=health.storage==='supabase'?'Supabase':'Belum diperiksa';
    return `<div class="settings-layout"><div class="card"><span class="settings-kicker">Kondisi website</span><h3>Status layanan</h3><div class="stat-list">${statLine('Sambungan akun',connected?'Tersambung':'Belum tersambung',connected?'green':'yellow')}${statLine('Internet',navigator.onLine?'Online':'Offline',navigator.onLine?'green':'yellow')}${statLine('Penyimpanan',storageLabel,health.ok?'green':'yellow')}${statLine('Waktu respons',health.latencyMs!==undefined?`${number(health.latencyMs)} ms`:'Belum diperiksa',health.ok?'green':'gray')}${statLine('Versi',health.version||'23.0.0','blue')}</div><button class="btn primary" id="checkWebsiteStatusBtn" style="margin-top:14px">Periksa Sekarang</button></div>
      ...
      <div class="card span-2"><span class="settings-kicker">Database Produksi</span><h3>${health.supabaseConfigured?'Supabase sudah aktif':'Supabase belum dihubungkan'}</h3><p class="muted">Penyimpanan produksi memakai tabel server dan ruang nota privat. Kunci layanan hanya disimpan pada environment Vercel, bukan di browser.</p><div class="notice ${health.supabaseConfigured?'good':'warning'}"><b>${health.supabaseConfigured?'Data utama diarahkan ke Supabase.':'Lengkapi SUPABASE_URL dan SUPABASE_SECRET_KEY di Vercel.'}</b><small>${health.migration?.migratedAt?`Pemindahan terakhir: ${escapeHtml(formatDateTime(health.migration.migratedAt))}.`:'Panduan pemasangan tersedia di paket final.'}</small></div>${health.supabaseConfigured?'<div class="notice good"><b>Supabase aktif.</b><small>Data operasional tersimpan di server Supabase dan tidak disalin penuh ke browser.</small></div>':''}</div>
  ...
  function advancedBackupSettings() {
    ...
    return `<div class="settings-layout"><div class="card"><span class="settings-kicker">Cadangan otomatis</span><h3>Jadwal harian aktif</h3><p class="muted">Supabase menyimpan data dan cadangan server. Sistem membuat cadangan pengaman sebelum perubahan penting.</p><div class="stat-list">${statLine('Jadwal','Setiap hari','green')}${statLine('Simpan maksimal','45 cadangan','blue')}</div><button class="btn primary" id="createServerBackupBtn">Buat Cadangan Sekarang</button><button class="btn soft" id="loadServerBackupsBtn">Muat Daftar</button></div>
  ...
  function updateTrackingStatus() {
    ...
    const secureWarn = (!window.isSecureContext && location.hostname !== 'localhost') ? '<div class="notice warn"><b>GPS butuh HTTPS/localhost.</b><small>Untuk deploy, buka dari HTTPS agar rekam jalur bisa aktif.</small></div>' : '';
