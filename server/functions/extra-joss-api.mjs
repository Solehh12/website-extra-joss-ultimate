...
async function loadData(){
  ...
  const initial=await prepareSeed(seed);
  initial.settings=initial.settings||{};
  initial.settings.dataVariant=VARIANT;
  initial.settings.storageMigration={
    source:'seed-v22',
    target:'supabase',
    mode:'vercel-first-run',
    migratedAt:new Date().toISOString()
  };
  await storage.setJson(DATA_KEY,initial,{onlyIfNew:true});
  return normalizeStoredData(await storage.getJson(DATA_KEY)||initial);
}
  ...
  try{
    ...
    if(request.method==='GET'&&action==='health'){
      if(!storage.supabaseConfigured){
        return json({ok:false,dynamic:true,storage:'supabase',version:'23.2.0',supabaseConfigured:false,dataVariant:VARIANT,error:'SUPABASE_URL dan SUPABASE_SECRET_KEY belum tersedia di environment Vercel.'},503);
      }
      try{
        const connection=await storage.ping();
        return json({ok:connection.ok,dynamic:true,storage:'supabase',version:'23.2.0',supabaseConfigured:true,dataVariant:VARIANT,latencyMs:connection.latencyMs});
      }catch(error){
        console.error('supabase-health', {
          name: error?.name || '',
        ...
        return json({
          ...
          dataVariant:VARIANT,
          error:error?.message||'Supabase tidak merespons.',
